...
Every worker receives a personalised weekly premium computed by a trained
**RandomForest regression model** (scikit-learn, 100 estimators), not a static
formula. This model is trained on 5,000 synthetic worker-week records that
reflect real seasonal, geographic, and behavioural patterns.

**Model Input Features:**

| Feature | Description | Source |
| --- | --- | --- |
| `city_risk` | Historical disruption frequency + flood vulnerability + platform outage rate, weighted composite | Pre-seeded lookup + internal claims history |
| `season_risk` | Computed from current calendar month; monsoon months (June–Sep) score 0.65–1.0 | Server datetime |
| `forecast_risk` | Rain probability for the upcoming week, correlated with season_risk | OpenWeatherMap live API |
| `income_volatility` | Std deviation / mean of the worker's weekly earnings across platforms | Worker's platform_account records |
| `platform_count` | Number of active platforms linked; more platforms = income diversification = lower risk | Worker's platform_account count |
| `weeks_active` | Worker tenure in weeks from created_at; longer tenure = more trust | Computed from worker.created_at |
| `persona_type` | Delivery category: FOOD=0, GROCERY=1, ECOMMERCE=2 | Worker profile |

**Training:**
Training data is generated by `ml/generate_training_data.py` and the model is
saved to `ml/risk_model.pkl` using joblib. Run `python ml/train_model.py` once
before starting the backend.

**Output:**
The model returns `weekly_premium` (₹) and a derived `risk_score` (0–1).
Feature importances from the trained model drive the XAI breakdown stored
in `weekly_cycles.risk_components` as JSONB.

**Explainability (XAI Layer):**
Because we use a RandomForest, feature importances are extracted directly
from `model.feature_importances_` — no post-hoc SHAP required. Each importance
is multiplied by the normalised feature value to produce a per-factor
contribution percentage. These are stored alongside every weekly cycle and
displayed to the worker as plain language:

> "Your premium this week is ₹116. This is mainly driven by:
> monsoon season risk in Chennai (+18%), rain forecast for your zone (+14%),
> and your city's historical disruption rate (+12%)."

**Example Calculation (Rahul, Chennai, August):**
- city_risk = 0.70, season_risk = 0.85, forecast_risk = 0.72,
  income_volatility = 0.40, platform_count = 2, weeks_active = 18, persona_type = 0
- Model input: [0.70, 0.85, 0.72, 0.40, 2, 18, 0]
- Model output: weekly_premium ≈ ₹119
- max_weekly_coverage = ₹4,500 × 0.62 = ₹2,790

**Loss Ratio Monitoring:**
Target range: 0.6–0.8. Admin dashboard flags loss ratio > 1.5 in real time.
The parametric simulator allows threshold recalibration without code deployment.
